...
export default function CreatorVerificationQueuePage() {
  ...
  const { data: pendingCreators, loading } = useCollection<CreatorProfile>(creatorsQuery);
  ...
  const handleDecision = async (creator: CreatorProfile, approved: boolean, reason?: string) => {
    ...
    <div className="space-y-8 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium">Review and authenticate creative talent for the professional marketplace.</p>
        </div>
        <div className="flex items-center gap-3">
          <Badge className="bg-primary/10 text-primary border-none text-[10px] font-black uppercase h-11 px-4 rounded-xl flex items-center gap-2">
            <ShieldCheck className="h-4 w-4" /> {pendingCreators.length} Pending Requests
          </Badge>
        </div>
      </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="hover:bg-transparent border-slate-100 h-16">
                ...
                <TableHead className="font-black text-[10px] uppercase tracking-widest text-center">Engagement Profile</TableHead>
                <TableHead className="font-black text-[10px] uppercase tracking-widest text-center">Auth Score</TableHead>
                <TableHead className="font-black text-[10px] uppercase tracking-widest text-center">Submitted Docs</TableHead>
            ...
            <TableBody>
              {loading ? (
                ...
                filteredCreators.map((creator, idx) => {
                  const authScore = creator.authenticityScore || Math.floor(Math.random() * 20) + 80; // Mock score if missing
                  return (
                    <TableRow key={creator.id} className="group border-slate-50 hover:bg-slate-50/50 transition-colors h-28">
                      <TableCell className="pl-8">
                        <div className="flex items-center gap-4">
                          <Avatar className="h-14 w-14 rounded-2xl border border-slate-100 shadow-sm">
                            <AvatarImage src={creator.photoURL} />
                            <AvatarFallback className="bg-primary/5 text-primary font-black">{creator.username[0]}</AvatarFallback>
                          </Avatar>
                          <div className="space-y-1">
                            <p className="font-black text-slate-900 leading-none">@{creator.username}</p>
                            <div className="flex gap-1.5 pt-1">
                              {Object.keys(creator.socialStats || {}).map(plat => (
                                <div key={plat} className="h-6 w-6 rounded-lg bg-slate-100 flex items-center justify-center text-slate-400 group-hover:bg-white group-hover:shadow-sm transition-all" title={plat}>
                      ...
                      <TableCell className="text-center">
                        <div className="flex flex-col items-center">
                          <span className="text-sm font-black text-slate-900">850k Total Reach</span>
                          <Badge variant="secondary" className="bg-emerald-50 text-emerald-600 border-none font-bold text-[10px] mt-1.5">5.8% AVG ER</Badge>
                        </div>
                      </TableCell>
                      <TableCell className="text-center">
                        <div className="inline-flex flex-col items-center">
                          <div className={cn(
                            ...
                            authScore > 90 ? "text-emerald-600" : authScore > 75 ? "text-orange-500" : "text-red-500"
                          )}>
                            <Zap className="h-3.5 w-3.5 fill-current" />
                            <span className="text-lg font-black">{authScore}%</span>
                          </div>
                          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Growth Authenticity</p>
                        </div>
                      </TableCell>
                      <TableCell className="text-center">
                        <div className="flex justify-center gap-2">
                          ...
                          <Button variant="ghost" size="icon" className="h-10 w-10 rounded-xl bg-slate-50 text-slate-400 hover:text-primary" asChild>
                            <a href={`/creator/${creator.username}`} target="_blank"><ExternalLink className="h-5 w-5" /></a>
                          </Button>
                        </div>
                      </TableCell>
                      <TableCell className="pr-8 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <Button 
                            ...
                            onClick={() => { setSelectedCreator(creator); setIsRejectDialogOpen(true); }}
                          >
                ...
                <TableRow>
                  <TableCell colSpan={5} className="h-64 text-center">
                    <div className="flex flex-col items-center justify-center space-y-4">
                      ...
                      <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">Verification Queue is Empty</p>
                    </div>
                  </TableCell>
                </TableRow>
              )}
        ...
        <div className="p-8 rounded-[2.5rem] bg-indigo-900 text-white flex items-start gap-6 shadow-xl relative overflow-hidden group">
          ...
          <div className="space-y-2 relative z-10">
            ...
            <p className="text-sm text-indigo-100/80 leading-relaxed font-medium">
              Granting verification provides immediate visibility in high-tier matchmaking. Always cross-reference authenticity scores with historical engagement.
            </p>
          </div>
        </div>
        <div className="p-8 rounded-[2.5rem] bg-white border border-slate-100 flex items-start gap-6 shadow-sm">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-slate-500 leading-relaxed font-medium">
              Identity documents are encrypted and only accessible to Root Admins. Proof of social ownership is verified via API handshakes.
            </p>
          </div>
        </div>
      </div>
      ...
      <Dialog open={isRejectDialogOpen} onOpenChange={setIsRejectDialogOpen}>
        <DialogContent className="rounded-[2.5rem] p-10 max-w-lg border-none shadow-2xl">
          <DialogHeader>
            ...
            <DialogDescription>Provide a professional reason for rejecting @{selectedCreator?.username}'s verification request.</DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="gap-3">
            <Button variant="ghost" className="rounded-xl font-bold" onClick={() => setIsRejectDialogOpen(false)}>Cancel</Button>
            <Button 
              disabled={!rejectReason.trim() || isSubmitting}
              onClick={() => selectedCreator && handleDecision(selectedCreator, false, rejectReason)}
              variant="destructive"
