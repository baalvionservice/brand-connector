...
export function DeadlineReminders() {
  ...
  const { data: campaigns } = useCollection<any>(
    userProfile?.role === 'BRAND' 
      ? query(collection(db, 'campaigns'), where('brandId', '==', `brand_${userProfile.id}`))
  ...
  const { data: applications } = useCollection<any>(
    userProfile?.role === 'BRAND'
      ...
      : query(collection(db, 'applications'), where('creatorId', '==', userProfile?.id || ''))
  );

  const { data: conversations } = useCollection<any>(
    userProfile?.id ? query(collection(db, 'conversations'), where('participantIds', 'array-contains', userProfile.id)) : null
  );
  ...
  useEffect(() => {
    ...
    const fetchDismissed = async () => {
      try {
        const snap = await getDocs(collection(db, 'users', userProfile.id, 'dismissed_reminders'));
  ...
  const activeReminders = useMemo(() => {
    if (loadingDismissed || !userProfile) return [];
    ...
    campaigns.forEach(c => {
      ...
      if (hoursLeft > 0 && hoursLeft <= 48) {
        ...
        list.push({
          ...
          link: `/dashboard/${userProfile.role.toLowerCase()}/campaigns/${c.id}`,
          icon: Clock
        });
      }
    });
    ...
    if (userProfile.role === 'BRAND' && applications.length > 0) {
      list.push({
    ...
    // --- MESSAGE CHECKS ---
    const unreadChats = conversations.filter(c => (c.unreadCounts?.[userProfile.id] || 0) > 0);
    if (unreadChats.length > 0) {
      list.push({
        ...
        link: `/dashboard/${userProfile.role.toLowerCase()}/messages`,
        icon: MessageSquare
      });
    }
    ...
    if (userProfile.role === 'CREATOR') {
      ...
      const hasHighBalance = true; // Simulating from userProfile or wallet hook
      if (hasHighBalance) {
        list.push({
          id: `payout_ready`,
          type: 'PAYOUT',
  ...
  }, [campaigns, applications, conversations, userProfile, dismissedIds, loadingDismissed]);
  ...
  const handleDismiss = async (reminderId: string) => {
    ...
    // Firestore update
    try {
      await setDoc(doc(db, 'users', userProfile.id, 'dismissed_reminders', reminderId), {
