...
export async function processPayout(db: Firestore, payoutId: string) {
  ...
  try {
    ...
    const creatorSnap = await getDoc(doc(db, 'creators', `creator_${tx.userId}`));
    
    await generateAndAttachInvoice(db, storage, { ...tx, ...updateData }, {
    ...
    await addDoc(collection(db, 'notifications'), {
      ...
      message: `₹${tx.amount.toLocaleString()} has been successfully transferred to your ${tx.payoutMethod?.type.toUpperCase()} account.`,
      type: 'PAYMENT',
      read: false,
      createdAt: new Date().toISOString()
    });
 ...
 */
export function getPayoutStep(status: TransactionStatus, method: string): number {
